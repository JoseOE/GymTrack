...
export function ProfileScreen() {
  ...
  const { activeTrainingLocation, profile, updateProfile, weeklyPlan } = useGymTrack();
  if (!profile || !weeklyPlan) return <Screen><ScreenHeader title="Perfil" subtitle="Cargando perfil local…" /></Screen>;
  return <ProfileForm activeTrainingLocation={activeTrainingLocation} email={user?.email ?? 'Correo no disponible'} emailConfirmed={Boolean(user?.email_confirmed_at)} key={profile.updatedAt} profile={profile} signOut={signOut} updateProfile={updateProfile} weeklyPlan={weeklyPlan} />;
}

function ProfileForm({ profile, weeklyPlan, activeTrainingLocation, email, emailConfirmed, updateProfile, signOut }: { profile: UserProfile; weeklyPlan: WeeklyPlan; activeTrainingLocation: import('@/domain/models').TrainingLocation | null; email: string; emailConfirmed: boolean; updateProfile: (profile: UserProfile) => Promise<void>; signOut: () => Promise<void> }) {
