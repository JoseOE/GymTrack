...
import type { MuscleGroup, WeeklyPlan, WeeklyPlanDayDraft, WeeklyPlanDraft } from '@/domain/models';
...
export function WeeklyPlanScreen() {
  const { loading, muscleGroups, resetWeeklyPlan, updateWeeklyPlan, weeklyPlan } = useGymTrack();
  ...
  return <WeeklyPlanEditor key={weeklyPlan.updatedAt} muscleGroups={muscleGroups} plan={weeklyPlan} resetWeeklyPlan={resetWeeklyPlan} updateWeeklyPlan={updateWeeklyPlan} />;
}

function WeeklyPlanEditor({ plan, muscleGroups, updateWeeklyPlan, resetWeeklyPlan }: { plan: WeeklyPlan; muscleGroups: MuscleGroup[]; updateWeeklyPlan: (draft: WeeklyPlanDraft) => Promise<void>; resetWeeklyPlan: () => Promise<void> }) {
  ...
  return (
    <Screen>
      <ScreenHeader title="Plan semanal" subtitle={`${target} ${target === 1 ? 'día cuenta' : 'días cuentan'} para tu objetivo`} action={<IconButton icon="close" label="Cerrar editor" onPress={close} />} />
      ...
      <PrimaryButton icon="save-outline" loading={saving} title="Guardar plan" onPress={() => void save()} />
